(function () {
  ...
  function teacherPhoto(teacher) {
    const key = JNA_IMG.fieldKey(teacher.field);
    return JNA_IMG.teachers[key];
  }

  function showDetail(teacher) {
    detailPhoto.textContent = '';
    detailPhoto.style.cssText = JNA_IMG.bgStyle(teacherPhoto(teacher));
  ...
  thumbsEl.innerHTML = teachers.map((t, i) =>
    `<button type="button" class="thumb-card ${i === 0 ? 'is-active' : ''}" data-id="${t.id}">
      <div class="thumb-img" style="${JNA_IMG.bgStyle(teacherPhoto(t))}">${t.initial}</div>
